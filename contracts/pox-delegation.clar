...
(define-public (delegate-stack-stx (users (list 30 (tuple
    ...
    (if true
      (ok (get result
        (fold delegate-stack-stx-inner users {start-burn-ht: start-burn-ht, pox-address: pox-address, result: (list)})))
...
(define-public (delegate-stack-stx-simple (users (list 30 principal))
    ...
    (if true
      (ok (get result
        (fold delegate-stack-stx-simple-inner users {start-burn-ht: start-burn-ht, pox-address: pox-address, result: (list)})))
      (err u1))) ;; defines uint as error type
...
(define-private (delegate-stack-stx-inner (details {user: principal, amount-ustx: uint})
                  (context (tuple
  ...
  (let ((user (get user details))
      ...
      (stack-delegated-amount user amount-ustx context)))

;; Stacks maximal amount of delegated stx tokens.
;; Stores the result in "grouped-stackers".
(define-private (delegate-stack-stx-simple-inner (user principal)
                  (context (tuple
                      (pox-address (tuple (hashbytes (buff 32)) (version (buff 1))))
                      (start-burn-ht uint)
                      (result (list 30 (response (tuple (lock-amount uint) (stacker principal) (unlock-burn-height uint)) uint))))))
...
(define-private (stack-delegated-amount (user principal) (amount-ustx uint)
                  (context (tuple
                      (pox-address (tuple (hashbytes (buff 32)) (version (buff 1))))
                      (start-burn-ht uint)
                      (result (list 30 (response (tuple (lock-amount uint) (stacker principal) (unlock-burn-height uint)) uint))))))
